...
function coreCalculate(RowsRawData, RowsDapAn) {
    ...
    let answerC = 0;
    listTotalPercentAnswer.forEach(element => {
        answerC += parseFloat(element);
    });

    answerC = (answerC / listTotalPercentAnswer.length).toFixed(2);
    console.log("ANSWERRR C", answerC);
    ...
    listExportDataFinal.forEach(element => {
        ...
        if (listKClone.length > 0) {
            ...
            listCaseK += "Case " + listKClone[0]["caseId"] + " (" + listKClone[0]["percent"] + "W), ";
            ...
            for (var i = 1; i < listKClone.length; i++) {
                ...
                listCaseK += "Case " + listKClone[i]["caseId"] + " (" + listKClone[i]["percent"] + "W), ";
            }
        }
        element["averageI"] = averageI.toFixed(2) + "W";
        ...
        if (caseCorrectMin == 0 && listTruebyUser.length > 0) {
            ...
            if (listOutputTrue.length > 0) {
                let listReUseTrue = "(Tính theo W USER-TRUE thấp nhất)\n" + "Test " + listOutputTrue[0]["testId"] + " : ";
                listReUseTrue += "Case" + listOutputTrue[0]["caseId"] + " (" + listOutputTrue[0]["percent"] + "W), ";
                ...
                for (var i = 1; i < listOutputTrue.length; i++) {
                    ...
                    listReUseTrue += "Case" + listOutputTrue[i]["caseId"] + " (" + listOutputTrue[i]["percent"] + "W), ";
                }
            ...
            if (listOutputTrue.length > 0) {
                let listReUseTrue = "(Tính theo W USER-TRUE cao nhất)\n" + "Test " + listOutputTrue[0]["testId"] + " : ";
                listReUseTrue += "Case" + listOutputTrue[0]["caseId"] + " (" + listOutputTrue[0]["percent"] + "W), ";
                let checkTest = listOutputTrue[0]["testId"];
                for (var i = 1; i < listOutputTrue.length; i++) {
                    if (checkTest !== listOutputTrue[i]["testId"]) {
        ...
        if (caseInCorrectMin == 0 && listFalsebyUser.length > 0) {
            ...
            if (listOutputFalse.length > 0) {
                let listReUseFalse = "(Tính theo W USER-FALSE thấp nhất)\n" + "Test " + listOutputFalse[0]["testId"] + " : ";
                listReUseFalse += "Case" + listOutputFalse[0]["caseId"] + " (" + listOutputFalse[0]["percent"] + "W), ";
                ...
                for (var i = 1; i < listOutputFalse.length; i++) {
                    ...
                    listReUseFalse += "Case" + listOutputFalse[i]["caseId"] + " (" + listOutputFalse[i]["percent"] + "W), ";
                }
            ...
            if (listOutputFalse.length > 0) {
                let listReUseFalse = "(Tính theo W USER-FALSE cao nhất)\n" + "Test " + listOutputFalse[0]["testId"] + " : ";
                listReUseFalse += "Case" + listOutputFalse[0]["caseId"] + " (" + listOutputFalse[0]["percent"] + "W), ";
                let checkTest = listOutputFalse[0]["testId"];
                for (var i = 1; i < listOutputFalse.length; i++) {
                    if (checkTest !== listOutputFalse[i]["testId"]) {
...
function checkDistinctNonSymmetric1(testId, userId, listExportData, session_no, case_true, total_case, objDapAn, type, listSysTrue, listSysFalse) {
    ...
    if (check.length == 0) {
        ...
        listExportData[objIndex].percent = ((listExportData[objIndex].numberCaseTrue / listExportData[objIndex].totalCase) * 100).toFixed(2) + "W";
    ...
    switch (dropdownValue) {
        case "1":
            if (calcNormal(objRowData, objDapAn) == true) {
                ...
                listCaseIdbyTestId[currentIndex]["percent"] = ((listCaseIdbyTestId[currentIndex]["totalUserTruth"] / listCaseIdbyTestId[currentIndex]["totalUser"]) * 100).toFixed(2) + "W";
