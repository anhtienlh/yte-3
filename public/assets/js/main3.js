...
function getResult() {
    ...
    if (!check || !selectTrue || !selectFalse || (!$("#txtMvalue").val() && (!$("#inlineCustomMValue").children("option:selected").val() || $("#inlineCustomMValue").children("option:selected").val() === '-1'))) {
...
function coreCalculate(RowsRawData, RowsDapAn) {
    ...
    console.log("LIST TOTAL PERCENNNNN ANSWER All TYPE", listTotalPercentAnswerByType);
    ...
    dropdownArrayValue.forEach(function(dropdownValue) {
        ...
        answerC = (answerC / listTotalPercentAnswerByType[typeOfValue].length).toFixed(2);
        if (selectedValueM && selectedValueM !== "-1") {
            valueM = calculateValueM(listTotalPercentAnswerByType[typeOfValue], selectedValueM);
    ...
    listExportDataFinal.forEach((element) => {
        ...
        element["averageI"] = averageIByType[element["type"]].toFixed(2) + "W";
        ...
        let listTruebyUser = listTrue
            .filter((obj) => obj["userId"] === element["userId"] && obj["type"] === element["type"])
            .map((obj) => obj);
...
function calculateValueM(listPercent, option) {
    ...
    switch (option) {
        ...
        case "4":
            ...
            break;
        case "5":
            value = (listPercent.reduce((a, b) => a + b, 0) * 25) / 100;
            break;
        case "6":
            value = (listPercent.reduce((a, b) => a + b, 0) * 75) / 100;
            break;
        default:
            break;
    }
