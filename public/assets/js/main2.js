...
function initRawDataTable(excelRowsRawData) {
  ...
  tableRowData = $("#tblRowData").DataTable({
    ...
    columns: [
      ...
      {
        title:
          '<div style="display: flex;" class="checkbox"><input type="checkbox" id="cbCaseDensity" onclick="onClick(event, \'cbCaseDensity\')"  />' +
          header[14] +
          '<img src="http://www.icone-png.com/png/39/38556.png" class="filterIcon" onclick="showFilter(event,\'' +
          "tblRowData" +
          "_" +
      ...
      {
        title:
          '<div style="display: flex;" class="checkbox"><input type="checkbox" id="cbLessionType"  onclick="onClick(event, \'cbLessionType\')"  />' +
          header[15] +
          '<img src="http://www.icone-png.com/png/39/38556.png" class="filterIcon" onclick="showFilter(event,\'' +
          "tblRowData" +
          "_" +
...
function coreCalculate(RowsRawData, RowsDapAn) {
  ...
  userFilter.forEach((user) => {
    ...
    let listExportDataClone = listExportData.filter(
      (obj) =>
                    ...
                    listAnswer: [
                      {
                        testId: arrUnique[0],
                        caseId:
                          "case " + listCaseIdbyTestId[objIndex]["caseId"],
                        percent: (
                          (listCaseIdbyTestId[objIndex]["totalUserTruth"] /
                            listCaseIdbyTestId[objIndex]["totalUser"]) *
                          100
  ...
  listExportDataFinal.forEach((element) => {
    ...
    let listD = listTrue.filter(
      (obj) =>
        obj["userId"] === element["userId"] && obj["type"] === element["type"]
    );
    ...
    if (listKClone.length > 0) {
      ...
      switch (element["type"]) {
        default:
          listCaseK +=
            listKClone[0]["caseId"] + " (" + listKClone[0]["percent"] + "%), ";
          break;
      }
    ...
    let listFalsebyUser = listFalse
      .filter((obj) => obj["userId"] === element["userId"])
      .map((obj) => obj);
...
function calcWofCaseIdRawData(data) {
  ...
  str.forEach((element) => {
    let objIndex = listClone.findIndex(
      (obj) => obj.testId === data["testId"] && obj.caseId === element
    );
    if (objIndex != -1) {
      let percent =
        (listClone[objIndex]["totalUserTruth"] /
      ...
      switch (data["type"]) {
        case "3":
          listCaseIdTrue +=
            ...
            "%)" +
            " - " +
            listClone[objIndex]["lesionId"] +
            " - " +
            listClone[objIndex]["truthOrder"] +
            " , ";
          listTrue.push({
            userId: data["userId"],
            testId: data["testId"],
        ...
        default:
          listCaseIdTrue +=
            "case " + element + " (" + percent.toFixed(2) + "%), ";
          listTrue.push({
            userId: data["userId"],
            testId: data["testId"],
            caseId: "case " + element,
  ...
  let listClone2 = listCaseIdbyTestId.slice(0);
  str2.forEach((element) => {
    let objIndex = listClone2.findIndex(
      (obj) => obj.testId === data["testId"] && obj.caseId === element
    );
    if (objIndex != -1) {
      let percent =
        (listClone2[objIndex]["totalUserTruth"] /
          listClone2[objIndex]["totalUser"]) *
        100;
      ...
      switch (data["type"]) {
        case "3":
          listCaseIdFalse +=
            ...
            "%)" +
            " - " +
            listClone2[objIndex]["lesionId"] +
            " - " +
            listClone2[objIndex]["truthOrder"] +
            " , ";
          listFalse.push({
            userId: data["userId"],
            testId: data["testId"],
...
function exportExcel() {
  ...
  dropdownArrayValue.forEach(function (dropdownValue) {
    var listExportDataFinalByDropdown = listExportDataFinal
      .filter((obj) => obj["type"] === dropdownObject[dropdownValue])
      .map((obj) => obj);
    var tab_value = [];
